...
# نکته مهم: مدل‌های gemini-1.5-* کاملاً از کار افتاده‌اند (خطای 404) و دیگر پاسخ نمی‌دهند.
# کتابخانه google-generativeai هم منسوخ شده؛ SDK فعلی گوگل «google-genai» است.
gemini_client = genai.Client(api_key=GEMINI_API_KEY)
GEMINI_MODEL = "gemini-2.5-flash"  # پایدار و پشتیبانی‌شده حداقل تا اکتبر 2026
...
async def handle_input_internal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if update.message.voice:
            # دانلود ویس و ارسال به جیمینای به‌عنوان Part صوتی معتبر (نه بایت خام)
            ...
            with tempfile.NamedTemporaryFile(suffix=".ogg", delete=False) as tmp:
                ...
                transcribe_prompt = (
                    "این پیام صوتی مربوط به یک تراکنش مالی (هزینه یا درآمد) است. "
                    "فقط متن گفته‌شده را به فارسی و عیناً پیاده‌سازی کن، بدون توضیح اضافه و بدون مقدمه."
                )
...
# ---------- راه‌اندازی اصلی ----------
def main():
    # JobQueue داخلی python-telegram-bot را فعال می‌کنیم (نیاز به APScheduler خارجی ندارد)
    ...
    # زمان‌بندی‌ها با JobQueue داخلی (context به‌درستی تزریق می‌شود)
    jq = app.job_queue
    # یادآوری شبانه — ساعت ۲۳:۰۰ تهران
    jq.run_daily(send_nightly_reminder, time=datetime.now(TIMEZONE).replace(hour=23, minute=0, second=0, microsecond=0).timetz())
    # پشتیبان‌گیری روزانه — ساعت ۰۰:۰۰ تهران
    jq.run_daily(auto_backup, time=datetime.now(TIMEZONE).replace(hour=0, minute=0, second=0, microsecond=0).timetz())
    # گزارش ماهانه — هر شب ۲۳:۵۹ بررسی می‌شود (خود تابع آخر ماه را چک می‌کند)
    jq.run_daily(send_monthly_villa_report, time=datetime.now(TIMEZONE).replace(hour=23, minute=59, second=0, microsecond=0).timetz())
